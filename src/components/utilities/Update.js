...
const Update = ({
  ...
  useEffect(() => {
    ...
    if (Config.view_type === VIEW_SUBJECT_TYPE) {
      setUpdatedID(id);
    } else {
      setUserID(id);
    }
  ...
  // Memoize payloadUpdated to avoid unnecessary re-renders
  const payloadUpdated = useMemo(() => {
    if (Config.view_type === VIEW_SUBJECT_TYPE) {
      return { ...payload, updatedID };
    } else {
      return { ...payload, userID };
    }
  }, [payload, userID, Config, updatedID]);
