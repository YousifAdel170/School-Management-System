...
if ($result_check_email->num_rows > 0) {
    ...
    if ($conn->query($sql_user) === TRUE) {
        ...
        $sql_teacher = "UPDATE teachingstaff SET name = '$name', gender = '$gender_label', hire_date = '$currentDate', supervisorID = '$supervisorID', 
        bod = '$BOD', specialization = '$specialization_lable', salary = '$salary', address = '$address' WHERE user_id = '$userID'";
        ...
        if ($conn->query($sql_teacher) === TRUE) {
            ...
            echo json_encode(["success" => false, "message" => "Failed to update Teacher information in the teachingstaff table." . $conn->error]);
        }
    } else {
        echo json_encode(["success" => false, "message" => "Failed to update Teacher information in the users table." . $conn->error]);
    }
}
